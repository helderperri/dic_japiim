...
$myfile = fopen("dic_".$dic_name."_sem_rev.tex", "w") or die("Unable to open file!");
...
try {
    ...
    $result3 = $link->query("SELECT * FROM sd_list ORDER BY sd_name_ref");
...
try {
    ...
    $result4 = $link->query("SELECT * FROM sds WHERE sd_id = '$sd_id'");
    ...
    if($result4->rowCount()>0){
      ...
      foreach ($result4 as $key => $row){
        ...
        try{
            $result = $link->query("SELECT * FROM sense_bundles WHERE sense_bundle_id = '$sense_bundle_id'");
          ...
          echo "Opps, houve um erro na sua busca<br><br>".$e->getMessage();
                  ...
                  //$entries[]= array($entry_id, );
                    ...
                    try {
                        ...
                        $result6 = $link->query("SELECT * FROM glosses WHERE sense_bundle_id = '$sense_bundle_id'");
                    ...
                    foreach ($result6 as $key => $row){                           
                        ...
                        $target_lang = $row['target_lang'];
                                ...
                                $entries[$count] = array('key' => $count, 'entry_id' => $entry_id, 'sense_bundle_id' => $sense_bundle_id, 'lang_code' => $lang_code, 'target_lang' => $target_lang, 'gloss' => $gloss);
...
array_walk(
    $entries,
    function (&$row){
        $row['gloss'] = iconv("UTF-8", "UTF-8", $row['gloss']);
    }
);

$sortValues = array_column($entries, 'gloss');
array_multisort($sortValues, SORT_LOCALE_STRING, $entries);
*/

usort($entries, 'order_entries');

//uasort($index_array, 'compareASCII');

// Sort the data with mid descending
// Add $data as the last parameter, to sort by the common key
//$m_gloss, SORT_STRING SORT_FLAG_CASE, $entries);


//$collator = collator_create('pt_BR');
//usort($entries, function($a, $b) use ($collator) {
  //  return $collator->compare($a['gloss'], $b['gloss']);
//});

//$coll = collator_create ( 'en_US' ); 
  
// Sort array according to its numeral value 
//collator_asort( $coll, $entries['gloss'], Collator::SORT_STRING );

//usort($entries, "cmp");
//array_multisort(array_column($entries, "gloss"), SORT_LOCALE_STRING, $entries);


 //usort($entries, build_sorter("gloss"));


foreach ($entries as $key => $row){    
    
    $vernacular_bundle = array();
    $example_bundle = array();
    $entry_output = "";
    $entry_id = $row['entry_id'];
    $sense_bundle_id=$row["sense_bundle_id"];    
    ...
    foreach ($result7 as $key => $row){                           
        ...
        try {
            ...
            $result8 = $link->query("SELECT * FROM forms WHERE form_bundle_id = '$form_bundle_id' ORDER BY vernacular COLLATE utf8mb4_general_ci");
            ...
            foreach ($result8 as $key => $row){
                ...
                try {
                    ...
                    $vernacular_bundle[] = "{".$vernacular." ".$phonemic_output."} ";
...
echo "Opps, houve um erro na sua busca<br><br>".$e->getMessage();
    ...
    $example_bundle[] = example_bundle_output_latex ($sense_bundle_id);




    $example_output = implode( "", $example_bundle );



    $vernacular_output = implode( "", $vernacular_bundle );


    $classes_output = classes_latex ($sense_bundle_id, $target_lang, $lang_code);


    

    $entry_output = "\\entry{".$gloss."}{\\textit{".$classes_output."}".$vernacular_output."".$example_output."}\n\n";



    fwrite($myfile, $entry_output);
    $entry_output = "";
    $vernacular_output = "";
    $example_output = "";
    $classes_output = "";
