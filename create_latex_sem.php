...
function cmp($a, $b)
{
    return strcmp($a["vernacular"], $b["vernacular"]);
}
...
function order_entries ($a, $b) {
    //    setlocale(LC_ALL, "pt_BR.UTF-8" );
        ...
        $arr = array($a['vernacular'], $b['vernacular']);
        collator_asort($collator, $arr, Collator::SORT_STRING);
      
        return array_pop($arr) == $a['vernacular'];
...
$myfile = fopen("dic_".$dic_name."_sem.tex", "w") or die("Unable to open file!");
              ...
              foreach ($result as $key => $row){
                  ...
                  //$entries[]= array($entry_id, );
                    ...
                    try {
                        ...
                        $result6 = $link->query("SELECT * FROM form_bundles WHERE entry_id = '$entry_id'");
                    ...
                    foreach ($result6 as $key => $row){                           
                        ...
                        try {
                            ...
                            $result5 = $link->query("SELECT * FROM forms WHERE form_bundle_id = '$form_bundle_id' ORDER BY vernacular COLLATE utf8mb4_general_ci");
                            ...
                            foreach ($result5 as $key => $row){
                                ...
                                $entries[$count] = array('key' => $count, 'entry_id' => $entry_id, 'form_id' => $form_id, 'vernacular' => $vernacular);
